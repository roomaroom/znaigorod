  ...
  def show
    ...
    respond_to do |format|
      format.html  do
        ...
        render layout: "organization_layouts/#{@organization.subdomain}" if !@organization.status.debtor? && !@organization.status.non_cooperation? &&!@organization.status.fresh? && @organization.subdomain? && template_exists?(@organization.subdomain, 'layouts/organization_layouts')
      end
