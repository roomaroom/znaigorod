class Culture < ActiveRecord::Base
  ...
  presents_as_checkboxes :category,
    :available_values => -> { HasSearcher.searcher(:cultures).facet(:culture_category).rows.map(&:value).map(&:mb_chars).map(&:capitalize).map(&:to_s) },
  ...
  presents_as_checkboxes :feature,
    :available_values => -> { HasSearcher.searcher(:cultures).facet(:culture_feature).rows.map(&:value) }

  presents_as_checkboxes :offer,
    :available_values => -> { HasSearcher.searcher(:cultures).facet(:culture_offer).rows.map(&:value) }
