...
handle_inviteables_search_click = ->
  $('.inviteables_search_results_wrapper').on 'click', (evt) ->
    return unless $(evt.target).closest('li').length
...
@init_invitations = ->
  ...
  $('.invitations_wrapper, .content .left, .accounts_list, .feature_wrapper, .additional_info_wrapper, .accounts_main_page_list').on 'ajax:success', (evt, response) ->
    ...
    handle_new_invitaion_link(target, response) if target.hasClass('invitation_link') || target.data('target') == '.invited' || target.data('target') == '.inviter'
    handle_reply_invitation(target, response) if target.hasClass('new_invitation')
    target.closest('li').remove() if target.hasClass('trash') && target.closest('.accounts_list').length
